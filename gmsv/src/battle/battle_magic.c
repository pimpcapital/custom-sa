...
*/
  snprintf(
      ...
      "B$|a%X|i%X|m%X|%X|%X|%X|s%X|t%X|l%X|%X|%X|%X|%X|%X|%X|o%X|o%X|o%X|s%X|%X|%X|%X|%X|",
...
#else
      snprintf(szCommand, sizeof(szCommand), "BD|r%X|%X|%X|d%X|p%X|", ToList[i], kind, Sign, Damage, petDamage);
#endif //_NOTRIDE_
      BATTLESTR_ADD(szCommand);
    }
...
static void BATTLE_GetAttr(int charaindex, int *pAt_Fire, int *pAt_Water, int *pAt_Earth, int *pAt_Wind,
                           int *pAt_None) {
    ...
    for(i = 0, j = basey - 1; j <= basey + 1; i++, j++) {
      ...
      for(k = 0; k < 5; k++) {
        ...
        if(ATTMAGIC_magic[magicattidx].siField[i][k] &&
           TRUE == BATTLE_TargetCheck(battleindex, CharTable[j][basex - 2 + k])) {
