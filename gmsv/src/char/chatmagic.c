...
void CHAR_CHAT_DEBUG_eventclean(int charaindex, char *message) {
  ...
  if(find) {
    ...
    sprintf(token, "Success!! Character Name:%s delete eventflag:[%d].", CHAR_getChar(charaindex_tmp, CHAR_NAME),
            eventno);
    CHAR_talkToCli(charaindex, -1, token, CHAR_COLORWHITE);
    // display
...
void CHAR_CHAT_DEBUG_eventsetnow(int charaindex, char *message) {
  ...
  if(find) {
    ...
    sprintf(token, "Success!! Character Name:%s eventnow_number:[%d].", CHAR_getChar(charaindex_tmp, CHAR_NAME),
            eventno);
    CHAR_talkToCli(charaindex, -1, token, CHAR_COLORWHITE);
    // display
    if(charaindex_tmp == charaindex)
...
void CHAR_CHAT_DEBUG_eventsetend(int charaindex, char *message) {
  ...
  if(find) {
    ...
    sprintf(token, "Success!! Character Name:%s eventend_number:[%d].", CHAR_getChar(charaindex_tmp, CHAR_NAME),
            eventno);
    CHAR_talkToCli(charaindex, -1, token, CHAR_COLORYELLOW);
    // display
    if(charaindex_tmp == charaindex)
...
void CHAR_CHAT_DEBUG_engineer(int charindex, char *message) {
  ...
  CHAR_send_P_StatusString(charindex, CHAR_P_STRING_DUELPOINT |
                                      ...
                                      CHAR_P_STRING_EXP | CHAR_P_STRING_LV | CHAR_P_STRING_HP |
                                      CHAR_P_STRING_LEARNRIDE);
...
void CHAR_CHAT_DEBUG_SHOWIP(int charindex, char *message) {
  ...
  if(strlen(message) > 0) {
    ...
    sprintf(token, "账号%s的IP是 %s", message, CONNECT_get_userip(i));
    CHAR_talkToCli(charindex, -1, token, CHAR_COLORYELLOW);
  }
}
...
void CHAR_CHAT_DEBUG_SET_FAME(int charaindex, char *message) {
  ...
  saacproto_ACFixFMData_send(acfd,
                             CHAR_getChar(i, CHAR_FMNAME), CHAR_getInt(i, CHAR_FMINDEX),
...
void CHAR_CHAT_DEBUG_petmakecf(int charaindex, char *message) {
  ...
  if(strlen(cdkey) > 0) {
...

    {
      ...
      rnd = ((cstr - (float) str) / 80.0 / 160.0 + (ctough - (float) tough) / 80.0 / 160.0 +
