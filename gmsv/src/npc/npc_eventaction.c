...
int Action_RunDoEventAction(int meindex, int toindex, char *buf1) {
  ...
  if(NPC_Util_GetStrFromStrWithDelim(buf1, "PartyCount", buf2, sizeof(buf2)) != NULL) {
    ...
    if(NPC_Util_GetStrFromStrWithDelim(buf2, Head, buf3, sizeof(buf3)) != NULL) {
      if(flg == 0) if(atoi(buf3) != partys) return FALSE;
      else if(flg == 1) if(atoi(buf3) < partys) return FALSE;
      else if(flg == 2) if(atoi(buf3) > partys) return FALSE;
      else if(flg == 3) if(atoi(buf3) <= partys) return FALSE;
      else if(flg == 4) if(atoi(buf3) >= partys) return FALSE;
      else if(flg == 5) if(atoi(buf3) == partys) return FALSE;
    }
  }
#endif
...
int NPC_ActionNewDelPet(int toindex, char *msg) {
  ...
  for(i = 0; i < CHAR_MAXPETHAVE; i++) {
    ...
    if(flg == 0) if(mypetlevel == petlev) cmp = TRUE;
    if(flg == 1) if(mypetlevel < petlev) cmp = TRUE;
    if(flg == 2) if(mypetlevel > petlev) cmp = TRUE;
...
int NPC_ActionFreeIfCheck(int meindex, int talker, char *buf, char *opt, int kosuu, int flg, int temp) {
  ...
  if(strcmp(buf, "FM") == 0) {
    if(CHAR_getInt(talker, CHAR_FMLEADERFLAG) > 0 &&
       CHAR_getInt(talker, CHAR_FMLEADERFLAG) != FMMEMBER_APPLY) if(CHAR_getWorkInt(talker, CHAR_WORKFMFLOOR) ==
                                                                    kosuu) {
      return TRUE;
    }
  }
...
int showString(int meindex, char *showstr, int flg) {  //广播
  char buf1[256];
...
int NPC_ActionManCountCheck(int toindex, int nums, int flg) {
  ...
  if(CHAR_getWorkInt(toindex, CHAR_WORKPARTYMODE) == CHAR_PARTY_NONE) {
    ...
    for(i = 0; i < CHAR_PARTYMAX; i++)
      if(CHAR_getWorkInt(toindex, CHAR_WORKPARTYINDEX1 + i) != -1) if(
          IsMale(CHAR_getWorkInt(toindex, CHAR_WORKPARTYINDEX1 + i)) == 1)
        tempcount++;
  }
...
int NPC_ActionWomanCountCheck(int toindex, int nums, int flg) {
  ...
  if(CHAR_getWorkInt(toindex, CHAR_WORKPARTYMODE) == CHAR_PARTY_NONE) {
    ...
    for(i = 0; i < CHAR_PARTYMAX; i++)
      if(CHAR_getWorkInt(toindex, CHAR_WORKPARTYINDEX1 + i) != -1) if(
          IsFemale(CHAR_getWorkInt(toindex, CHAR_WORKPARTYINDEX1 + i)) == 1)
        tempcount++;
  }
  print("\n检查女生人数:%d", tempcount);
  if(NPC_ActionBigSmallLastCheck(nums, tempcount, flg) == TRUE)
