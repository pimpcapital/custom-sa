...
static int NPC_RoomAdminNew_SetNextAuction(int meindex);

static int NPC_RoomAdminNew_WriteDataFile(int meindex, int pindex, char *roomname,
                                          char *passwd, int flg);
...
void NPC_RoomAdminNewLoop(int meindex) {
  ...
  hun = ceil((double) (CHAR_getWorkInt(meindex, CHAR_WORKINT_NEXTTIME) - NowTime.tv_sec) / 60.0);
  if(hun < 0) hun = 0;
...
static void NPC_RoomAdminNew_CreatePasswd(char *passwd, int passlen) {
#define NPC_ROOMADMINNEW_RAND()    ( (rand() /256) % 32768 )
  int i;
  char *candidates[] = {
...
static int NPC_RoomAdminNew_WriteDataFile(int meindex, int pindex,
                                          char *roomname,
                                          char *passwd, int flg) {
  char *cdkey;
  char charaname[32 * 2];
