...
void charLoadCallback(int ti, int auth, char *c0, char *c1, char *c2, char *c3, char *c4, int i0, int i1) {
  ...
  if(charindex < 0) {
    saacproto_ACCharLoad_send(ti, FAILED, "char nonexistent", mesgid, charindex);
    return;
  }
...
void charListCallback(int ti, int auth, char *c0, char *c1, char *c2, char *c3, char *c4, int i0, int i1) {
...
void charDeleteCallback(int ti, int auth, char *c0, char *c1, char *c2, char *c3, char *c4, int i0, int i1) {
  int charindex;
  char *id = c0;
  char *charname = c2;
  int mesgid = i0;
    ...
    if(index != -1) {
      if(fmcharindex == 0) {
        ...
        if(ChangeFMLeader(index, fmname, fmindex) >= 0) {
          if(ACDelFM(index, fmname, fmindex) >= 0) log("删除家族成员:%d 家族名:%s 家族索引:%d 人物:%s, 账号:%s, 家族人物索引:%d\n",
...
#ifdef _FMVER21
        if(ACMemberLeaveFM(index, fmname, fmindex, charname, 0, 0, fmcharindex) >= 0) log(
            "ACMemberLeaveFM index:%d 家族名:%s 家族索引:%d 人物:%s, 账号:%s, 家族人物索引:%d\n",
            index, fmname, fmindex, charname, id, fmcharindex);
#else
