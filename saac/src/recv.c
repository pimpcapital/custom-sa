...
void saacproto_ACCharLoad_recv(int ti, char *id, char *pas, char *charname, int lock, char *opt, int mesgid) {
...
void saacproto_LockLogin_recv(int fd, char *id, char *ip, int flag) {
  switch(flag) {
    case 0:
      if(strlen(id) > 0) if(sasql_del_lock(id)) log("成功解除锁定账号:%s\n", id);
      if(strlen(ip) > 0) if(sasql_del_lock(ip)) log("成功解除锁定IP:%s\n", ip);
      break;
    case 1:
      if(strlen(id) > 0) if(!sasql_chehk_lock(id)) if(sasql_add_lock(id)) log("成功锁定账号:%s\n", id);
      if(strlen(ip) > 0) if(!sasql_chehk_lock(ip)) if(sasql_add_lock(ip)) log("成功锁定IP:%s\n", ip);
      break;
    default:
      break;
  }
